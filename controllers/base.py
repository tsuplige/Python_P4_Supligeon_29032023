...
class Controllers:
    ...
    def manage_tournament(self, tournament):
        ...
        while tournament.current_round < tournament.number_of_round:
            ...
            while self.next_round == True :
                self.tournament_menu(str(tournament.current_round), match_list, tournament)
        ...
        self.end_tournament_menu(tournament, tournament.name, tournament.locale, tournament.tournament_begin_date, tournament.tournament_end_date, tournament.description, tournament.round_list)
    ...
    def end_tournament_menu(self,tournament, name, locale, tournament_begin_date, tournament_end_date, description, round_list):

        

        menu_input = self.view.tournament_end_menu_prompt(name, locale, tournament_begin_date, tournament_end_date)

        while not menu_input:
            menu_input = self.view.tournament_menu_prompt()
        if menu_input == '1':
            ask_to_continue = self.view.print_players_list_by_point(self.players)
            self.end_tournament_menu(tournament, name, locale, tournament_begin_date, tournament_end_date, description, round_list)
        elif menu_input == '2':
    ...
    def save_tournament_data(self, new_tournament):
        ...
        for i in range(len(self.tournament_list)-1):
            if self.tournament_list[i].name == new_tournament.name:
                del self.tournament_list[i]
                print ("Suprimmé")
            self.tournament_list.append(new_tournament)
    ...
    def load_tounament(self, tournament):
        ...
        if not tournament.is_not_finish :
